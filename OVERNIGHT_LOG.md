...
- **resume session complete (initial 7)** — 7 commits landed (BUG-4, BUG-7, B9, B3, A7, A8+A9, B5); branch now 19 ahead of `main`. Final gate: full `grainTests` suite green (34 passed / 0 failed / 3 skipped = the A11-blocked indexer tests). **Deferred (documented):** A1 (`SpendingAnalytics`→struct), A6 + A10 (relationship delete-rules + schema migration) — all schema-touching, need a launch test.
- **B8 + B7 promoted; wrapped up (user-directed)** — B8 thermal demo images verified live in the proof/split view; B7 fresh captures promoted to the canonical `screenshots/` set (Home, Detail, Proof, Analytics, Index) with gallery captions updated. 03·Scan / 06·Item Watch / 08·Retailers left pre-redesign (computer-use disconnected mid-session + no native sim-tap fallback, so they couldn't be recaptured — flagged pending in the gallery README). Branch pushed; PR opened to `main`.
